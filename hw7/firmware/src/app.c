    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_MOUSE_EMULATE:
           ...
           // APP_ProcessSwitchPress();
            ...
            //short imu_data[7];
            read_IMU(imu_data);
            x_dir = (int8_t)(((float)imu_data[4]*2.0/32768.0)*5);
            y_dir = (int8_t)(((float)imu_data[5]*2.0/32768.0)*5);
            ...
            if(appData.emulateMouse)
            {
                ...
                if(movement_length > 50)
                {
                    ...
                    appData.xCoordinate =x_dir;//(int8_t)dir_table[vector & 0x07] ;
                    appData.yCoordinate =y_dir;//(int8_t)dir_table[(vector+2) & 0x07];
                    vector ++;
                    movement_length = 0;
                }
            }
