...
void LCD_Draw_Character(unsigned short x, unsigned short y, char character,unsigned short fore_colour, unsigned short back_colour){
    char i,j, value, temp;
    for (i=0;i<5;i++){
        value = ASCII[character-32][i];
        for (j=0;j<8;j++){
            temp=value>>j;
            temp = temp&0b1;
            if (temp == 1){
                LCD_drawPixel(x+i,y+j,fore_colour);
            }
            else {
                LCD_drawPixel(x+i,y+j,back_colour);
            }
        }
    }
...
void LCD_Draw_String(unsigned short x, unsigned short y, char *string,unsigned short fore_colour, unsigned short back_colour){
    int i = 0;
    while(string[i] != 0){
        LCD_Draw_Character(x,y,string[i],fore_colour, back_colour);
        x = x+6;
        i++;
        if (x > 123){// new line if at the end of the row
            x = 0;
            y = y + 9;                    
        }
    }
